...
function kurzusokBetoltese() {
    fetch(kurzusApiUrl)
        ...
        .then(data => {
            kurzusokElem.innerHTML = ''; // Lista ürítése az újratöltés előtt
            data.forEach(kurzus => {
...
function diakokBetoltese() {
    fetch(diakApiUrl)
        .then(response => response.json())
        .then(data => {
            diakokElem.innerHTML = ''; // Lista ürítése az újratöltés előtt
            data.forEach(diak => {
                const li = document.createElement('li');
    ...
    fetch(kurzusApiUrl)
        ...
        .then(data => {
            ...
            if (kurzusExists) {
                ...
                .then(() => {
                    kurzusokBetoltese(); // Az új kurzus mentése után az adatokat újratöltjük
                    kurzusFormMegerese(); // A formot elrejtjük
                })
    ...
    fetch(diakApiUrl)
        ...
        .then(data => {
            ...
            if (diakExists) {
                ...
                .then(() => {
                    diakokBetoltese(); // Az új diák mentése után az adatokat újratöltjük
                    diakFormMegerese(); // A formot elrejtjük
                })
...
function kurzusTorles(kurzusId) {
    ...
    .then(() => {
        kurzusokBetoltese(); // A kurzus törlése után az adatokat újratöltjük
    })
...
function diakTorles(diakId) {
    ...
    .then(() => {
        diakokBetoltese(); // A diák törlése után az adatokat újratöltjük
    })
...
// Az oldal betöltésekor az adatokat betöltjük
